...
from flask import Flask, request, jsonify, Blueprint, send_file, make_response
...
FILE_NAME('Project_"""
            ...
            + """','"""
            + get_timestamp()
            + """',('Author'),(''),'BIM Workbench','IFC4','Unknown');
FILE_SCHEMA(('IFC4'));
ENDSEC;
DATA;
#1=IFCPROJECT('"""
            + project_id
            + """',$,'BIM Project',$,$,$,$,(#10),#1);
#10=IFCGEOMETRICREPRESENTATIONCONTEXT($,'Model',3,$,#20,$);
#20=IFCAXIS2PLACEMENT3D(#30,$,$);
#30=IFCCARTESIANPOINT((0.,0.,0.));
ENDSEC;
END-ISO-10303-21;
"""
        )
        response = make_response(ifc_content)
        response.headers["Content-Disposition"] = (
            f'attachment; filename="project_{project_id}.ifc"'
        )
        response.headers["Content-Type"] = "application.ifc"
        response.headers["X-Export-ID"] = generate_id()
...
EOF
"""
        response = make_response(dxf_content)
        response.headers["Content-Disposition"] = (
            f'attachment; filename="project_{project_id}.dxf"'
        )
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 600" width="800" height="600">
  ...
  <rect x="50" y="50" width="700" height="500" fill="none" stroke="#000000" stroke-width="2"/>
  <text x="400" y="300" text-anchor="middle" font-family="Arial" font-size="24">Project: """
            + project_id
            + """</text>
  <text x="400" y="340" text-anchor="middle" font-family="Arial" font-size="14">Generated by BIM Workbench</text>
</svg>
"""
        )
        response = make_response(svg_content)
        response.headers["Content-Disposition"] = (
            f'attachment; filename="project_{project_id}.svg"'
        )
