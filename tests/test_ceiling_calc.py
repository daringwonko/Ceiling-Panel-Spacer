...
def test_basic_functionality():
    ...
    print(
        f"  Panel size: {layout1.panel_width_mm:.1f}mm × {layout1.panel_length_mm:.1f}mm"
    )
    print(
        f"  Layout: {layout1.panels_per_row} × {layout1.panels_per_column} = {layout1.total_panels} panels"
    )
    ...
    print(
        f"  Panel size: {layout2.panel_width_mm:.1f}mm × {layout2.panel_length_mm:.1f}mm"
    )
    print(
        f"  Layout: {layout2.panels_per_row} × {layout2.panels_per_column} = {layout2.total_panels} panels"
    )
    ...
    for i, (alt_layout, efficiency) in enumerate(alternatives, 1):
        print(
            f"  Option {i}: {alt_layout.panels_per_row}×{alt_layout.panels_per_column} panels "
    ...
    try:
        ...
        print(
            f"  Material: {material.name} - {material.color} - ${material.cost_per_sqm}/m²"
        )
