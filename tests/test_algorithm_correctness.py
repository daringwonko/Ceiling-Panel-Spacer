    ...
    for length, width, perim, panel, desc in test_cases:
        try:
            ...
            assert layout.panel_width_mm <= 2400, (
                f"Panel width {layout.panel_width_mm:.0f}mm exceeds max"
            )
            assert layout.panel_length_mm <= 2400, (
                f"Panel length {layout.panel_length_mm:.0f}mm exceeds max"
            )
            ...
            print(
                f"✓ {desc:<30} {layout.panels_per_row}×{layout.panels_per_column} = {layout.total_panels:>3} panels "
...
def test_edge_cases():
    ...
    edge_cases = [
        ...
        (
            ...
            100,
            False,
            "Gap too large (3000mm on 5000mm = exceeds half)",
        ),
    ...
    print(
        ...
        if failed == 0
        else f"✗ {failed} EDGE CASES FAILED"
    )
    return failed == 0
...
def test_real_world_scenarios():
    ...
    for name, (length, width, perim, panel) in scenarios.items():
        try:
            ...
            print(
                f"{name:<30} | {layout.total_panels:>3} panels ({layout.panels_per_row}×{layout.panels_per_column}) | "
                f"{layout.panel_width_mm:>6.0f}×{layout.panel_length_mm:>6.0f}mm | {coverage_pct:>5.1f}% coverage"
            )

            # Verify all constraints met
            assert layout.panel_width_mm <= 2400 and layout.panel_length_mm <= 2400, (
                "Panel size constraint violated"
            )
...
def test_cost_calculations():
    ...
    print(
        f"Layout: {layout.total_panels} panels ({layout.panel_width_mm:.0f}×{layout.panel_length_mm:.0f}mm)\n"
    )
    ...
    print(
        f"{'Config':<30} | {'Material':<12} | {'Waste':<12} | {'Labor':<12} | {'Total':<12}"
    )
    print("-" * 85)
    ...
    for waste, labor, desc in test_configs:
        ...
        print(
            f"{desc:<30} | ${costs['material_cost']:>10,.2f} | ${costs['waste_cost']:>10,.2f} | "
            f"${costs['labor_cost']:>10,.2f} | ${costs['total_cost']:>10,.2f}"
        )

        # Verify calculations
        assert (
            costs["total_material_cost"] == costs["material_cost"] + costs["waste_cost"]
        ), "Cost calculation error"
        if labor:
            assert costs["labor_cost"] > 0, (
                "Labor cost should be > 0 when multiplier set"
            )
...
def test_optimization_strategies():
    ...
    print(
        f"{'Strategy':<20} | {'Panels':<8} | {'Panel Size':<15} | {'Description':<30}"
    )
    print("-" * 75)
    ...
    for strategy in strategies:
        ...
        desc = (
            ...
            else "Minimize seams (fewer panels)"
        )

        print(
            f"{strategy:<20} | {layout.total_panels:>6} | {panel_size:<15} | {desc:<30}"
        )

        # Verify constraints still met
        assert layout.panel_width_mm <= 2400, (
            f"Strategy '{strategy}' violated constraints"
        )
