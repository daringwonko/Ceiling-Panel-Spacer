...
class Roof:
    ...
    def get_geometry(self) -> Dict[str, Any]:
        ...
        if not self.is_valid_wire():
            return {"type": "error", "message": "Invalid wire: must be closed with at least 3 points"}
    ...
    def get_preview_geometry(self) -> Dict[str, Any]:
        ...
        for i in range(len(self._base_wire) - 1):
            ...
            lines.append({
                "start": (p1[0] + self.position[0], p1[1] + self.position[1], self.position[2]),
                "end": (p2[0] + self.position[0], p2[1] + self.position[1], self.position[2]),
            })
        ...
        if self._roof_type == RoofType.GABLE:
            ...
            lines.append({
                "start": (centroid[0] - 500, centroid[1], self.position[2] + ridge_height),
                "end": (centroid[0] + 500, centroid[1], self.position[2] + ridge_height),
            })
        elif self._roof_type == RoofType.HIP:
            # Ridge point for hip
            lines.append({
                "start": (centroid[0], centroid[1], self.position[2] + ridge_height),
