...
export function BeamPropertiesPanel({ beam, onUpdate, onValidate }: BeamPropertiesProps) {
  const { updateObject } = useBIMStore()
  const [localProps, setLocalProps] = useState<BeamProperties>(beam.properties)
  ...
  const volume = calculateBeamVolume(
    localProps.length,
    localProps.profileWidth,
    localProps.profileHeight
  )
  ...
  useEffect(() => {
    ...
    onValidate?.(result)
  }, [localProps.length, localProps.profileWidth, localProps.profileHeight, localProps.elevation, onValidate])

  // Handle property change with debounced update
  const handlePropertyChange = useCallback((
    property: keyof BeamProperties,
    value: any
  ) => {
  ...
  const handleApply = useCallback(() => {
    ...
    updateObject(beam.id, {
      properties: localProps
    })
    setHasChanges(false)
  }, [beam.id, localProps, validation.valid, updateObject])

  // Reset to original values
  const handleReset = useCallback(() => {
    setLocalProps(beam.properties)
    setHasChanges(false)
  }, [beam.properties])
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <GripHorizontal className="h-5 w-5 text-savage-primary" />
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <PropertyInput
                label="Profile Width"
                value={localProps.profileWidth}
                type="number"
                unit="mm"
                min={100}
                max={600}
                step={10}
                onChange={(value) => handlePropertyChange('profileWidth', parseFloat(value) || 0)}
              />
              
              <PropertyInput
                label="Profile Height"
                value={localProps.profileHeight}
                type="number"
                unit="mm"
                min={150}
                max={1200}
                step={10}
                onChange={(value) => handlePropertyChange('profileHeight', parseFloat(value) || 0)}
              />
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              ...
              <Tooltip>
                ...
                <TooltipContent>
                  <p>Update beam length based on start and end point positions</p>
                </TooltipContent>
              </Tooltip>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <PropertyInput
                ...
                value={localProps.startConnection}
                type="select"
                options={['pinned', 'fixed', 'roller', 'none']}
                onChange={(value) => handlePropertyChange('startConnection', value as BeamConnection)}
              />
              
              <PropertyInput
                label="End Connection"
                value={localProps.endConnection}
                type="select"
                options={['pinned', 'fixed', 'roller', 'none']}
                onChange={(value) => handlePropertyChange('endConnection', value as BeamConnection)}
              />
            </div>
            ...
            <div className="text-xs text-savage-text-muted bg-slate-800 p-2 rounded">
              ...
              <div className="space-y-1">
                <div><strong>Pinned:</strong> Allows rotation, resists translation</div>
                <div><strong>Fixed:</strong> Resists rotation and translation</div>
                <div><strong>Roller:</strong> Allows rotation and horizontal movement</div>
                <div><strong>None:</strong> Free end</div>
              </div>
            </div>
          </div>
                  ...
                  return material ? (
                    <div className="space-y-1">
                      ...
                      {material.tensileStrength && (
                        <div>Tensile Strength: {material.tensileStrength} MPa</div>
                      )}
                    </div>
                  ) : null
                })()}
